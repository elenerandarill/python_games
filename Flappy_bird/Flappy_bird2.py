...
def draw_pipe(x,y,h):	#rysuje rure	
	...
	for i in range (0, 2):
		if y < 0:
			my_pen.forward(50)	#grubosc rur na 'x' to 50
			my_pen.left(90)
			my_pen.forward(h)
			my_pen.left(90)
		else:
...
def game_loop():
	...
	if game.bird_y > 360 or game.bird_y < -400:
		...
		return
	for pipe in game.pipes_all:
		if pipe.y < 0:		#rury dolne
			'''punkt (-18, y)'''
			if (game.bird_x - 18) > pipe.x and (game.bird_x - 18) < (pipe.x + 50) \
				and game.bird_y < (pipe.y + pipe.h):
				game_over()
				#print("hit at -18, y d")
				return
			'''punkt (18, y)'''
			...
			'''punkt (18, 36)'''
			if (game.bird_x + 18) > pipe.x and (game.bird_x + 18) < (pipe.x + 50) \
					and (game.bird_y + 36) < (pipe.y + pipe.h):
				game_over()
				#print("hit at 18,36 d")
				return
	...
	for pipe in game.pipes_all:	#usuwa rury, ktore wyszly poza ekran
		if pipe.x < -500:
